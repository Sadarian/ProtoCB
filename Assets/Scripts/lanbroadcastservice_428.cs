...
public class LANBroadcastService : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            && Time.time > fTimeLastMessageSent + fIntervalMessageSending)
        {
            ...
            objUDPClient.Send(objByteMessageToSend, objByteMessageToSend.Length, new IPEndPoint(IPAddress.Broadcast, scrConnectionManager.iUDPPortNumber));
            // Restart the timer
    ...
    private void StartBroadcastingSession()
    {
        ...
        // Create the client
        objUDPClient = new UdpClient(scrConnectionManager.iUDPPortNumber);
